...
-- 02 将分组结果写入到tdb_goods_cates数据表

INSERT tdb_goods_cates (cate_name) SELECT goods_cate FROM tdb_goods GROUP BY goods_cate;

-- 03 通过tdb_goods_cates数据表来更新tdb_goods表
...
--  001 通过CREATE...SELECT来创建数据表并且同时写入记录
...
-- 将刚才查询到的子类数量更新到tdb_goods_types数据表----------连接-（子查询-连接）----

UPDATE tdb_goods_types AS t1
  INNER JOIN ( SELECT p.type_id,p.type_name,count(s.type_name) AS children_count FROM tdb_goods_types AS p
...
-- ----------------------------------删除重复记录（模拟利用两个表删除数据，结合使用连接和子查询）----------------------------------
DELETE t1 FROM tdb_goods AS t1
  LEFT JOIN (SELECT goods_id,goods_name FROM tdb_goods GROUP BY goods_name
